...
async function verifyUser(ctx, next) {
  // 1.取出用户名和密码
  const { name, password,effectBeginTime,effectEndTime,cellphone} = ctx.request.body
  ...
  if(!cellphone) {
    return ctx.app.emit('error', CELLPHONE_IS_REQUIRED, ctx)
  }

  // 3.有效时间处理
  if(!effectBeginTime) {
    return ctx.app.emit('error', EFFECT_BIGIN_TIME_IF_REQUIRE, ctx)
  }

  if(!effectEndTime) {
    return ctx.app.emit('error',EFFECT_END_TIME_IF_REQUIRE, ctx)
  }

  //3.1 判断开始结束时间是否符合要求
  const flag = compareTime(effectBeginTime,effectEndTime)
  if(!flag) {
      return ctx.app.emit('error',BEGIN_TIME_LOWER_END_TIME, ctx)
  }
